...
class WebhookSendRequest(BaseModel):
    """
    Standard payload sent by Main API WebhookCallbackService
    when pretending to talk to a Userbot.
    """
    agent_id: str   # This maps to 'account_id' in our worker
    chat_id: str    # The Telegram User ID to reply to
...
async def webhook_send_message(req: WebhookSendRequest):
    ...
    try:
        # Use our existing manager to send the message
        msg_id = await telegram_manager.send_message(
