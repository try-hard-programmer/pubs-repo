  ...
  */
  ...
  try {
    ...
    if (
      ...
      console.log(
        `   🔎 [TRACE] Processing ${sendOptions.mentions.length} mentions:`,
      ...
      for (let m of sendOptions.mentions) {
        ...
        if (typeof m === "string") {
          ...
          // [FIX] Detect LIDs even if they have wrong suffix (like @c.us) or correct @lid
          const isLid =
            finalId.includes("@lid") ||
            (finalId.startsWith("2") && finalId.length >= 12); // LIDs are usually long

          if (isLid) {
            try {
              // Ensure we use the correct lookup format
              const lookupId = finalId.includes("@c.us")
                ? finalId.replace("@c.us", "@lid")
                : finalId;

              console.log(
                `   🔄 [TRACE] Attempting to resolve LID: ${lookupId}`,
              );
              const contact = await client.getContactById(lookupId);

              if (contact && contact.number) {
                const phoneId = `${contact.number}@c.us`;
                console.log(
                  `   ✅ [TRACE] Mapped LID ${lookupId} -> ${phoneId}`,
                );

                // Optional: Replace raw ID in body if present
                if (typeof contentToSend === "string") {
                  const rawId = lookupId.split("@")[0];
                  if (contentToSend.includes(`@${rawId}`)) {
                    // If we found the Real Name, we assume Python already put the name in the body.
                    // But if the body still has the raw ID, we swap it for the phone number as a fallback.
                    contentToSend = contentToSend.replace(
                      new RegExp(`@${rawId}`, "g"),
                      `@${contact.number}`,
                    );
                    console.log(
                      `   📝 [TRACE] Replaced raw @${rawId} with @${contact.number} in body`,
                    );
                  }
                }
                finalId = phoneId;
                ...
                console.log(
                  `   ⚠️ [TRACE] Could not resolve LID: ${lookupId} (No contact/number returned)`,
                );
                // If resolution failed, ensure we use @lid suffix so WA at least tries to tag the ghost ID
                finalId = lookupId;
              }
            } catch (e) {
              console.error(
                `   ❌ [TRACE] Error resolving LID ${finalId}:`,
                e.message,
              );
            }
          }
    ...
    // 3. SAFE SEND HELPER (This stops the crash)
    const safeSend = async (cId, cData, cOptions) => {
      try {
        return await client.sendMessage(cId, cData, cOptions);
      } catch (error) {
        const msg = error?.message || String(error) || "Unknown error";

        // [CRASH FIX] Known non-fatal errors - message likely sent
        ...
        const isIgnorable = ignorableErrors.some((err) =>
          msg.toLowerCase().includes(err.toLowerCase()),
        );

        if (isIgnorable) {
          console.warn(
            `⚠️ [Anti-Crash] Ignored error: "${msg.substring(
              0,
              80,
            )}...". Message likely sent.`,
          );
          return { success: true, warning: msg, recovered: true };
        }

        // Log but don't crash
        console.error(`❌ [safeSend] Unhandled error: ${msg}`);
        throw error;
      }
    };
    ...
    switch (contentType) {
      case "string":
        ...
        // [FIX] Use safeSend instead of client.sendMessage
        messageOut = await safeSend(chatId, contentToSend, sendOptions);
        break;
      ...
      case "MessageMediaFromURL": {
        ...
        messageOut = await safeSend(chatId, messageMediaFromURL, sendOptions);
        break;
      }
      case "MessageMedia": {
        ...
        messageOut = await safeSend(chatId, messageMedia, sendOptions);
        break;
      }
      case "Location": {
        ...
        messageOut = await safeSend(chatId, location, sendOptions);
        break;
      }
      case "Buttons": {
        ...
        messageOut = await safeSend(chatId, buttons, sendOptions);
        break;
      }
      case "List": {
        const list = new List(
        ...
        messageOut = await safeSend(chatId, list, sendOptions);
        break;
      }
      case "Contact": {
        ...
        messageOut = await safeSend(chatId, contact, sendOptions);
        break;
      }
      case "Poll": {
        const poll = new Poll(
        ...
        messageOut = await safeSend(chatId, poll, sendOptions);
        break;
      }
      default:
