...
const restoreSessions = () => {
  try {
    // 🧹 SELF-HEALING: Remove stale locks to prevent 'Code: 21' crashes
    console.log("🧹 Startup: Scanning for stale Chrome locks...");
    try {
      if (fs.existsSync(sessionFolderPath)) {
        execSync(`find ${sessionFolderPath} -name 'SingletonLock' -delete`);
        console.log("✅ Stale locks removed successfully.");
      }
    } catch (err) {
      console.log(
...
const setupSession = (sessionId) => {
  try {
    ...
    // Disable the delete folder from the logout function
    ...
    // 🛡️ ZOMBIE PROTECTION: Limit QR Retries
    let qrRetryCount = 0;
    ...
    client.on("qr", async () => {
      ...
      if (qrRetryCount >= MAX_QR_RETRIES) {
        ...
        try {
          ...
          console.error(`Failed to delete zombie folder: ${e.message}`);
        }
      }
    });
    ...
    waitForNestedObject(client, "pupPage")
      .then(() => {
        const restartSession = async (sessionId) => {
          ...
          console.log(`✅ Session ${sessionId} restart sequence complete.`);
        };
  ...
  checkIfEventisEnabled("message").then((_) => {
    client.on("message", async (message) => {
      ...
      const bodyPreview = message.body
        ...
        : "[media/empty]";
      const isGroup = chat.isGroup ? "GROUP" : "DM";
      const hasMention = message.mentionedIds?.length > 0 ? "mention" : "";
      const hasQuote = message.hasQuotedMsg ? "quote" : "";
      const quotedPart = message.hasQuotedMsg
        ? `quoted=${message._data?.quotedParticipant || "unknown"}`
        : "";
      const flags = [hasMention, hasQuote, quotedPart]
        .filter(Boolean)
        .join(",");

      console.log(
        `[${timestamp}] ${isGroup} ${sender} > "${bodyPreview}" ${flags ? `[${flags}]` : ""}`,
      );
