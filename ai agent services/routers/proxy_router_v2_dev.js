...
const API_CONFIGS = {
  ...
  gemini: {
    ...
    chatModel: "gemini-2.0-flash", // ✅ stable
    visionModel: "gemini-2.0-flash", // ✅ stable
...
async function getGeminiEmbeddings(texts) {
  ...
  // 1. CHUNK YOUR DATA (Limit is 100 per Google's Docs)
  const CHUNK_SIZE = 100;
  const chunks = [];
  for (let i = 0; i < allTexts.length; i += CHUNK_SIZE) {
    chunks.push(allTexts.slice(i, i + CHUNK_SIZE));
  }
  ...
  for (const chunk of chunks) {
    const batchRequests = chunk.map((text) => ({
      model: `models/${config.embeddingModel}`, // Ensure this is "models/text-embedding-004"
      content: { parts: [{ text: text || "." }] },
    }));

    // 2. USE THE CORRECT URL (v1beta requires the model name in the path)
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${config.embeddingModel}:batchEmbedContents?key=${apiKey}`;
    ...
    const embeddings = response.data.embeddings.map((emb) => emb.values);
    allEmbeddings = allEmbeddings.concat(embeddings);
    totalTokens += chunk.reduce(
      (acc, t) => acc + Math.ceil((t?.length || 0) / 4),
      0,
    );
  }
  ...
  return {
    object: "list",
    data: allEmbeddings.map((values, i) => ({
      object: "embedding",
      embedding: values,
      index: i,
    })),
...
router.post("/chat/filemanager", authenticate, async (req, res) => {
  ...
  // ========== INLINE HELPER FUNCTIONS (ONLY OpenAI) ==========
  ...
  const calculateTokenCostLocal = (input, output) => {
  ...
  const logCreditUsageLocal = async (orgId, queryType, response, startTime) => {
    const responseTime = Date.now() - startTime;
    const credits = CREDIT_COSTS[queryType];
    const tokenCost = calculateTokenCostLocal(
      response.usage?.prompt_tokens || 0,
      response.usage?.completion_tokens || 0,
    );
  ...
  const handleOpenAILocal = async (
    ...
    // Handle legacy "files" array - append images to last user message
    if (hasFiles) {
      const lastUserIndex = messages
        .map((m, i) => ({ role: m.role, index: i }))
        .reverse()
  ...
  try {
    ...
    const userId = organization_id || "default_org";
    const queueKey = `queue_filemanager:${userId}`; // TERPISAH dari /chat
    ...
    const job = {
      jobId,
      requestId,
      provider: "openai", // Fixed OpenAI only
      messages,
      files,
      temperature,
      organization_id,
    ...
    // Start dedicated worker jika belum ada (INLINE WORKER LOGIC)
    ...
    // Wait for result (same as /chat)
    const result = await waitForResultLocal(jobId, 180000);

    if (result.success) {
      res.json(result.data);

      // Fire and forget ticket update (sama seperti sekarang)
      if (category?.toLowerCase() === "low" && ticket_id) {
        const aiResponse = result.data.choices[0].message.content;
        updateTicket(ticket_id, category, ticket_categories, aiResponse);
      }
